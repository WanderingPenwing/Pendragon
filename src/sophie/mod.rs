        ...
        match self {
            ...
            Commande::Modifie(nom, expression) => {
                let valeur = match variables[nom].type_element() {
                    TypeElement::Entier => {
                        Element::Entier(nombre::calcule_nombre(expression.clone(), variables)?)
                    }
                    TypeElement::Texte => {
                        Element::Texte(texte::calcule_texte(expression.clone(), variables)?)
                    }
                    TypeElement::Booleen => {
                        Element::Booleen(booleen::calcule_booleen(expression.clone(), variables)?)
                    }
                };
                variables.insert(nom.to_string(), valeur);
            }
