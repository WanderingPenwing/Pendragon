...
fn calcul_texte() {
    ...
    match possible_expression {
        Ok(expression) => match sophie::texte::calcule_texte(expression, &HashMap::new()) {
            Ok(texte) => {
                let vrai_texte = format!(
                    "hello{}therevraitroisdeux\t\n",
                    sophie::nombre::nombre_comme_texte(a * b)
                );
                assert_eq!(
                    texte, vrai_texte,
                    "Calcul d'expression (texte) donne un mauvais résultat : {}",
                    texte
                );
            }
            Err(raison) => {
                panic!(
                    "Calcul d'expression (texte) échoué, avec l'erreur : {}",
                    raison
                );
            }
        },
...
fn conversion_texte() {
    ...
    match pendragon.elements_texte(texte) {
        Ok(expression) => {
            ...
            assert_eq!(
                expression[0],
                Element::Texte(texte[1..texte.len() - 1].into()),
                "Calcul d'expression (texte) donne un mauvais résultat : {}",
                texte
            );
        }
...
fn erreur_conversion_texte() {
    ...
    for texte in textes {
        ...
        let ErreurPendragon::TexteInvalide(_) = raison else {
            panic!(
                "Erreur imprévue pour convertir le texte '{}' : {}",
                texte, raison
            );
        };
    }
...
fn conversion_booleen_texte() {
    for b in [true, false].iter() {
        ...
        match pendragon::booleen::texte_comme_booleen(&texte) {
            // Convert text back to number
            Ok(booleen) => {
                assert_eq!(
                    Element::Booleen(*b),
                    booleen,
                    "Booleen inexact : {}, texte : {}",
                    b,
                    texte
                );
            }
            Err(raison) => {
                panic!(
                    "Conversion échouée pour : {}, avec l'erreur : {}",
                    b, raison
                );
            }
        }
...
fn calcul_booleen() {
    ...
    for configuration in configurations {
        let possible_expression = pendragon.elements_booleen(&format!(
            "{} et non ouvre la parenthèse {} ou non {} ferme la parenthèse ou non {} et {}",
        ...
        match possible_expression {
            Ok(expression) => match sophie::booleen::calcule_booleen(expression, &HashMap::new()) {
                Ok(booleen) => {
                    let resultat = configuration.0 && !(configuration.1 || !configuration.2)
                        || !configuration.3 && configuration.4;
                    assert_eq!(
                        booleen, resultat,
                        "Calcul d'expression (booleen) donne un mauvais résultat : {}",
                        booleen
                    );
                }
                Err(raison) => {
                    panic!(
                        "Calcul d'expression (booleen) échoué, avec l'erreur : {}",
                        raison
                    );
                }
            },
...
fn comparaison_booleen() {
    ...
    for (a, b) in [(1, 4), (2, 2), (3, 1), (0, 3)] {
        let possible_expressions = vec![
            pendragon.elements_booleen(&format!(
                "non six plus {} est supérieur à deux fois {}",
                sophie::nombre::nombre_comme_texte(a),
                sophie::nombre::nombre_comme_texte(b)
            )),
            pendragon.elements_booleen(&format!(
                "six plus {} est inférieur à deux fois {}",
                sophie::nombre::nombre_comme_texte(a),
                sophie::nombre::nombre_comme_texte(b)
            )),
            pendragon.elements_booleen(&format!(
                "six plus {} est supérieur ou égal à deux fois {}",
                sophie::nombre::nombre_comme_texte(a),
                sophie::nombre::nombre_comme_texte(b)
            )),
            pendragon.elements_booleen(&format!(
                "non six plus {} est inférieur ou égal à deux fois {}",
                sophie::nombre::nombre_comme_texte(a),
                sophie::nombre::nombre_comme_texte(b)
            )),
            pendragon.elements_booleen(&format!(
            ...
            for c in 0..5 {
                for d in 1..5 {
                    for e in 0..5 {
                        ...
                        match possible_expression {
                            Ok(expression) => {
                                match sophie::booleen::calcule_booleen(
                                    expression.clone(),
                                    &HashMap::new(),
                                ) {
                                    Ok(booleen) => {
...
fn erreur_calcul_booleen() {
    ...
    for texte in textes_invalide {
        ...
        let ErreurPendragon::OrdreCalculBooleen(_, _, _) = raison else {
            panic!(
                "Devrait détecter une erreur de calcul booléen pour '{}', a déclenché : {}",
                texte, raison
            );
        };
    }
...
fn conversion_nombres_texte() {
    ...
    .iter()
    {
        ...
        match pendragon::nombre::texte_comme_nombre(&texte) {
            // Convert text back to number
            Ok(nombre) => {
                assert_eq!(
                    Element::Entier(*i),
                    nombre,
                    "Nombre inexact : {}, texte : {}",
                    i,
                    texte
                );
            }
...
fn calcul_nombre() {
    ...
    let d = 45678;
    let e = 2;
    let possible_expression = pendragon.elements_nombre(&format!(
        "{} fois {} plus ouvre la parenthèse {} moins {} ferme la parenthèse divisé par {}",
        sophie::nombre::nombre_comme_texte(a),
        sophie::nombre::nombre_comme_texte(b),
        sophie::nombre::nombre_comme_texte(c),
        sophie::nombre::nombre_comme_texte(d),
    ...
    match possible_expression {
        Ok(expression) => match sophie::nombre::calcule_nombre(expression, &HashMap::new()) {
            Ok(nombre) => {
                assert_eq!(
                    nombre,
                    a * b + (c - d) / e,
                    "Calcul d'expression (entier) donne un mauvais résultat : {}",
                    nombre
                );
            }
            Err(raison) => {
                panic!(
                    "Calcul d'expression (entier) échoué, avec l'erreur : {}",
                    raison
                );
            }
        },
...
fn erreur_calcul_nombre() {
    ...
    for texte in textes_invalide {
        ...
        let ErreurPendragon::OrdreCalculEntier(_, _, _) = raison else {
            panic!(
                "Devrait détecter une erreur de calcul entier pour '{}', a déclenché : {}",
                texte, raison
            );
        };
    }
...
fn nombre_invalide_et() {
    ...
    let ErreurPendragon::NombreInvalide(_) = raison else {
        panic!(
            "Devrait détecter une erreur de nombre invalide pour 'et', a déclenché : {}",
            raison
        );
    };
}
