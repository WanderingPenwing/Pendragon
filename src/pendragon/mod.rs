...
impl Pendragon {
            ...
            for phrase in phrases {
                if let Some(contenu) = phrase.strip_suffix(".") {
                    ...
                    match self.compile_commande(contenu) {
                        ...
                        Err(raison) => erreurs.push(ErreurCompilation::nouvelle(
                            index_ligne,
                            ligne.into(),
                            raison,
                        )),
                ...
                return Err(vec![ErreurCompilation::nouvelle(
                    index_ligne,
                    ligne.into(),
                    ErreurPendragon::ManquePonctuation,
                )]);
    ...
    fn definis(&mut self, arguments: &str) -> Result<Commande, ErreurPendragon> {
        ...
        self.programme
            .ajoute_variable(variable_nom.clone(), variable_type.clone())?;

        Ok(Commande::Definis(variable_nom, variable_type))
    }
    ...
    fn demande(&mut self, arguments: &str) -> Result<Commande, ErreurPendragon> {
        ...
        Ok(Commande::Demande(variable_nom))
    }
}
...
fn nom_de_variable(arguments: &str, separateur: &str) -> Result<(String, String), ErreurPendragon> {
    let parties = if separateur.is_empty() {
        vec![arguments, ""]
    } else {
        ...
        for commentaire in commentaires {
            match pendragon.compile(commentaire.into()) {
                Ok(_) => assert_eq!(
                    ...
                    0,
                    "Le commentaire '{}' ne devrait pas générer de commande",
                    commentaire
                ),
                Err(erreurs) => {
                    let affichage_erreurs = erreurs
                        .iter()
                        .map(|item| format!("{}", item.raison()))
                        .collect::<Vec<_>>()
                        .join("\n\n");
                    panic!(
                        "Erreur de compilation du commentaire '{}' : \n{}",
        ...
        for commentaire in commentaires {
            ...
            if erreurs.len() > 1 {
                let affichage_erreurs = erreurs
                    ...
                    .join("\n\n");
                panic!(
                    "Plus d'erreurs que prévu pour le commentaire '{}' : {:?}",
                    commentaire, affichage_erreurs
                )
            }
            let ErreurPendragon::CommandeInconnue(_) = erreurs[0].raison() else {
                panic!(
                    "Erreur inattendue de compilation du commentaire '{}' : {}",
                    commentaire,
                    erreurs[0].raison()
                );
            };
    ...
    fn ponctuation_invalide() {
        ...
        for texte in textes {
            ...
            if !manque_ponctuation {
                panic!(
                    "Il devrait y avoir une erreur de ponctuation dans le texte '{}'",
                    texte
                );
            }
        }
        ...
        for texte in textes {
            ...
            if !commande_inconnue {
                let affichage_erreurs = erreurs
                    ...
                    .join("\n\n");
                panic!(
                    "La commande devrait être inconnue '{}', erreurs : \n{}",
