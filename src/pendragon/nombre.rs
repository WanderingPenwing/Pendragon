...
pub const NOMS_UNITES: [&str; 10] = [
    "", "un", "deux", "trois", "quatre", "cinq", "six", "sept", "huit", "neuf",
];
pub const NOMS_UNITES_DIX: [&str; 10] = [
    "dix", "onze", "douze", "treize", "quatorze", "quinze", "seize", "dix-sept", "dix-huit",
    "dix-neuf",
];
pub const NOMS_DIZAINES: [&str; 9] = [
    "",
    "dix",
    "vingt",
    "trente",
...
pub const NOMS_SEPARATEURS: [&str; 7] = [
            ...
            match element {
                "plus" => {
                    while let Some(operateur) = pile_operateurs.last() {
                        if *operateur == Operateur::Plus
                            || *operateur == Operateur::Moins
                            || *operateur == Operateur::Fois
                            || *operateur == Operateur::Divise
                        {
                ...
                "ouvre-la-parenthese" => {
                    if !precede_par_operation {
                        return Err(ErreurPendragon::OrdreCalculEntier(
                            "opérateur".into(),
                            element_precedent.into(),
                            "l'ouverture de parenthèse".into(),
                        ));
                    }
                ...
                "ferme-la-parenthese" => {
                    if precede_par_operation {
                        return Err(ErreurPendragon::OrdreCalculEntier(
                            "nombre".into(),
                            element_precedent.into(),
                            "la fermeture de parenthèse".into(),
                        ));
                    }
                ...
                autre => {
                    ...
                    if format_de_variable(autre) {
                        self.programme
                            .variable_est_de_type(autre, TypeElement::Entier)?;
