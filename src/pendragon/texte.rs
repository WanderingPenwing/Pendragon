...
impl Pendragon {
                ...
                if !pile_texte.is_empty() {
                    ...
                    if let Some(dernier_element) = expression.last() {
                        if *dernier_element != Element::Operateur(Operateur::Puis) {
                            return Err(ErreurPendragon::TexteInvalide(format!(
                                "attends un 'puis' entre '{}' et '{}'",
    ...
    pub fn puis(
        ...
        expression: &[Element],
        pile_inconnu: &[String],
    ) -> Result<Vec<Element>, ErreurPendragon> {
        let Some(premier_element) = pile_inconnu.first() else {
            ...
            return Err(ErreurPendragon::TexteInvalide(
                "il manque un élément avant le puis".into(),
            ));
        };
        ...
        if pile_inconnu.len() == 1 && format_de_variable(premier_element) {
            return Ok(vec![Element::Variable(
                premier_element.into(),
                self.programme.variable(premier_element)?,
            )]);
        }
